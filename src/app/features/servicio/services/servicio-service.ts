...
export interface ServicioDetalle { id: number; cantidad: number; producto: string; categoria?: string | null; precio_unitario?: number | string; nota?: string | null; opciones: string[]; opciones_estilo?: { nombre: string; color_fondo?: string | null }[]; listo: boolean; servido?: boolean; llevando_por_id?: number | null; llevando_por?: string | null; entregado_por?: string | null; agregado_por?: string | null; }
...
export class ServicioService {
  ...
  desconfirmar(detalleId: number, token?: string) { return this.http.patch<{ detalle_id: number; listo: boolean; servido: false }>(`${this.api}/detalles/${detalleId}/desconfirmar`, {}, this.opciones(token)); }
  listarMesas(token?: string) { return this.http.get<{ mesas: Mesa[] }>(`${this.api}/mesas`, this.opciones(token)); }
  actualizarMesa(id: number, mesaId: number, token?: string) {
    return this.http.patch<{ orden_id: number; mesa: string }>(`${this.api}/fichas/${id}/mesa`, { mesa_id: mesaId }, this.opciones(token));
  }
  actualizarCubiertos(id: number, entregados: boolean, token?: string) { return this.http.patch<{ cubiertos_entregados: boolean }>(`${this.api}/fichas/${id}/cubiertos`, { cubiertos_entregados: entregados }, this.opciones(token)); }
  ...
  sincronizarReservas(sesion_id: string, items: { producto_id: number; cantidad: number }[], opciones: { modificador_opcion_id: number; cantidad: number }[], token?: string) {
    return this.http.post(`${this.api}/reservas-stock/sincronizar`, { sesion_id, items, opciones }, this.opciones(token));
  }
  liberarReservas(sesion_id: string, token?: string) {
    return this.http.delete(`${this.api}/reservas-stock`, { ...this.opciones(token), body: { sesion_id } });
  }
  ...
  agregarAdicional(id: number, data: { producto_id: number; cantidad: number; nota?: string | null; modificador_opcion_ids: number[]; reserva_sesion?: string }, token?: string) {
